    ...
    Camera(cam)
{
  ...
  static gvar3<int> gvnTargetFeatureCount("Tracker.TargetFeatureCount", 3000, SILENT);
  size_t nTargetFeatureCount = *gvnTargetFeatureCount;
  for (int i = 0; i < LEVELS; ++i) {
    aLevels[i].SetTargetFeatureCount(nTargetFeatureCount * 0.9, nTargetFeatureCount * 1.1);
    nTargetFeatureCount /= 4;
  }
}
